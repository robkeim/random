...
namespace AdventOfCode
{
    ...
    //
    // Wandering around a secure area, you come across a datalink port to a new part of the network.After briefly scanning it for interesting files, you find one
    // file in particular that catches your attention.It's compressed with an experimental format, but fortunately, the documentation for the format is nearby.
    //
    ...
    // (10x2). To decompress this marker, take the subsequent 10 characters and repeat them 2 times.Then, continue reading the file after the repeated data.The marker
    // itself is not included in the decompressed output.
    //
    // If parentheses or other characters appear within the data referenced by a marker, that's okay - treat it like normal data, not a marker, and then resume
    ...
    // - X(8x2)(3x3)ABCY becomes X(3x3)ABC(3x3)ABCY (for a decompressed length of 18), because the decompressed data from the (8x2) marker (the (3x3)ABC) is skipped and
    ...
    //
    // In version two, the only difference is that markers within decompressed data are decompressed.This, the documentation explains, provides much more substantial
    // compression capabilities, allowing many-gigabyte files to be stored in only a few kilobytes.
    //
    // For example:
    //
    // - (3x3)XYZ still becomes XYZXYZXYZ, as the decompressed section contains no markers.
    // - X(8x2)(3x3)ABCY becomes XABCABCABCABCABCABCY, because the decompressed data from the(8x2) marker is then further decompressed, thus triggering the(3x3)
    //   marker twice for a total of six ABC sequences.
    // - (27x12)(20x12)(13x14)(7x10)(1x12)A decompresses into a string of A repeated 241920 times.
    // - (25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN becomes 445 characters long.
    // Unfortunately, the computer you brought probably doesn't have enough memory to actually decompress the file; you'll have to come up with another way to get
    ...
    public static class Day09
    {
        public static int GetDecompressedLength(string input)
        {
            ...
            while (curCharIndex < input.Length)
            {
                var curChar = input[curCharIndex];
                if (curChar != '(')
                {
                ...
                {
                    var closeIndex = input.IndexOf(")", curCharIndex, StringComparison.InvariantCulture);
                    var repeatString = input.Substring(curCharIndex, closeIndex - curCharIndex + 1);

                    var match = repeatRegex.Match(repeatString);
                    var length = int.Parse(match.Groups[1].ToString());
                    var num = int.Parse(match.Groups[2].ToString());

                    var stringToRepeat = input.Substring(curCharIndex + match.Length, length);
                    ...
                    curCharIndex += match.Length + length;
                }
            }

            var resultString = result.ToString().Replace(" ", string.Empty);
