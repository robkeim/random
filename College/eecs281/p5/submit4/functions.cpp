...
void simulatedAnnealing(vector<vector<string> > &clusters, vector<vector<int> > &matrix, hash_map<string, int> &index)
{
	...
	for (int i = 0; i < clusters.size(); i++)
	{
		...
		if (clusters.at(i).size() > 1)
		{
			for (int j = 0; j < clusters.at(i).size(); j++)
			{
				for (int k = 1; k < clusters.at(i).size(); k++)
				{
	...
	*/
	...
	for (int iterations = 0; iterations < 1000; iterations++)
	{
		...
		int clust = -1, col = -1, potentialDiam = -1;
		for (int i = 0; i < clusters.size(); i++)
		{
			for (int j = 0; j < clusters.at(i).size(); j++)
			{
		...
		int debugDiam = 0;
		if (clusters.at(randClust).size() > 1)
		{
			hash_map <string, int>::iterator It7 = index.find(clusters.at(randClust).at(0));
			for (int i = 1; i < clusters.at(randClust).size(); i++)
			{
				hash_map <string, int>::iterator It8 = index.find(clusters.at(randClust).at(i));
				int tmp = matrix.at(It7->second).at(It8->second);
				if (tmp > debugDiam)
				{
					debugDiam = tmp;
				}
				...
				if (debugDiam != diameters.at(randClust))
				{
					cout << "incorrect updating of diameters\n";
					//exit(1);
				}	
		
		//cout << potentialDiam << "|" << diameters.at(randClust) << endl;
		if (potentialDiam > debugDiam)  // check with probability of reassigning
		{
			double T = static_cast<double>(debugDiam) / 5.0;
			double I = static_cast<double>(potentialDiam) - static_cast<double>(diameters.at(randClust));
			double exponent = -1 * I / T;
			double probability = exp(exponent) * 100;
			double randProb = (rand()%101);
			
			//rand()%101) / 100.0 < exp (-I/T)
			
			if (randProb < probability)
			{
				clusters.at(randClust).push_back(files.at(randFile));
				clusters.at(clust).erase(clusters.at(clust).begin() + col);
				diameters.at(randClust) = potentialDiam;
				
				if (clusters.at(clust).size() > 1)
				{
					int newDiam = 0;
						hash_map <string, int>::iterator It1 = index.find(clusters.at(clust).at(0));
						for (int i = 1; i < clusters.at(clust).size(); i++)
						{
							hash_map <string, int>::iterator It2 = index.find(clusters.at(clust).at(i));
							int tmp = matrix.at(It1 -> second).at(It2 -> second);
							if (tmp > newDiam)
							{
								newDiam = tmp;
						...
						diameters.at(clust) = newDiam;
				}
				else
				{
					diameters.at(clust) = 0;
				}
			}
			
			// make sure to updates diameters vector!!!
		}
		else // automatically reassign it
		{
			clusters.at(randClust).push_back(files.at(randFile));
						clusters.at(clust).erase(clusters.at(clust).begin() + col);
					int	newDiam2 = 0;
						hash_map <string, int>::iterator It3 = index.find(clusters.at(randClust).at(0));
						for (int i = 1; i < clusters.at(randClust).size(); i++)
						{
							hash_map <string, int>::iterator It4 = index.find(clusters.at(randClust).at(i));
							int tmp = matrix.at(It3 -> second).at(It4 -> second);
							if (tmp > newDiam2)
							{
								newDiam2 = tmp;
							}
						...
						diameters.at(randClust) = newDiam2;
			//diameters.at(randClust) =

					int newDiam3 = 0;
						hash_map <string, int>::iterator It5 = index.find(clusters.at(clust).at(0));
						for (int i = 1; i < clusters.at(clust).size(); i++)
						{
							hash_map <string, int>::iterator It6 = index.find(clusters.at(clust).at(i));
							int tmp = matrix.at(It5 -> second).at(It6 -> second);
							if (tmp > newDiam3)
							{
								newDiam3 = tmp;
							}
						}
						diameters.at(clust) = newDiam3;
			// diameters.at(clust) = 
		}		
	}	
	return;
