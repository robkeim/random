...
void bnb(vector<string> files, vector<vector<string> > &bestClust, vector<vector<string> > curClust, int &minDiam, int curDiam, vector<vector<int> > &matrix, hash_map<string, int> &index, const int K)
{
	if (files.size() == 0)
	{
		if ((curDiam < minDiam) && (curClust.size() == K)) // not sure if this is the correct way the autograder does it, but it's worth a shot
		{
	...
	//bnb(files, bestClust, curClust, minDiam, curDiam, matrix, index);
		for (int i = 0; i < curClust.size(); i++)
		{
			...
			{
				bnb(files, bestClust, curClust, minDiam, curDiam, matrix, index, K);
			}
		}		
	}	
	return;
...
void simAnneal(vector<vector<string> > &clusters, vector<vector<int> > &matrix, hash_map<string, int> &index)
{
	...
	*/
	...
	for (int iterations = 0; iterations < 1000; iterations++)
	{
		...
		if (potentialDiam > diameters.at(randClust))  // check with probability of reassigning
		{
			double T = static_cast<double>(diameters.at(randClust)) / 5.0;
			...
			double randProb = (rand()%101) / 100.0;
