...
*/
	...
	for (int i = 1; i < clusters.size(); i++)
	{
		...
		for (int j = 0; j < clusters.at(i).size(); j++)
		{
			hash_map <string, int>::iterator It2 = index.find(clusters.at(i).at(j));
			int tmpDist = matrix.at(It1 -> second).at(It2 -> second);
			//cout << "temp dist " << tmpDist << endl;
			if (tmpDist > tmpMax)
			{
				tmpMax = tmpDist;
			}
		}
		//cout << tmpMax << " ? " << minDist << endl;
		if (tmpMax < minDist)
		{
			//cout << tmpMax << " < " << minDist << endl;
			minDist = tmpMax;
		...
		if (tmpMax == minDist)
		{
			//mergesort(clusters.at(0).begin(), clusters.at(0).end(), compStr);
	...
	//cout << endl << endl << minClustNum << endl << endl;
...
//	}
	...
	for (int i = 0; i < bitVectors.size(); i++)
	{
		index[bitVectors.at(i).first] = i; // Not sure if this is working correctly but this could be a problem area
