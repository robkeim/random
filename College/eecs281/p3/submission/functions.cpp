...
// hash all of the words in a given file
{
...
// read each directory and call hash function
{
	DIR *dir = NULL;
...
// Simple bubble sort
{
		...
		while (elt1 != rel.end())
		{
			if ((elt1 -> rel) > (elt2 -> rel)) // check for relevances out of order
			{
				string tmp;
				int tmp2;
				tmp = (elt1 -> name);
			...
			if ((elt1 -> rel) == (elt2 -> rel)) // if equal relevances organize them alphabetically
			{
...
// finds the first word in a query
{
	string tmp = "";
...
// handles all of the behavior in the query mode
{
