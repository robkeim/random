...
an Error exception is thrown with an error message if the index is not within a valid range.
...
/*** NOTE: remove all comments that are instructions to you, the student, designated with starting with "/***" 
...
public:
	...
	// are neither copied nor modified. No memory allocation/deallocated is done.
	...
	static void set_messages_wanted(bool in_messages_wanted)
		{messages_wanted = in_messages_wanted;}
	
private:
	/*** To get a consistent object size to match the instructors solution, 
	you must have exactly two ints and a pointer as  private (non-static) member variables; 
	private member functions are completely your choice. */
	
	/* Variables for monitoring functions - not part of a normal implementation */
	/*** These are used here for demonstration and testing purposes. You must have and support these members */
	...
	static bool messages_wanted;	// whether to output ctor/dtor/operator= messages
