...
void Warship::update()
{
	...
	// If the Warship is not longer afloat or it's target is no longer afloat
	// then stop the attack
    if (!is_afloat() || !get_target() || !get_target()->is_afloat())
    {
        stop_attack();
        return;
    }  
...
void Warship::attack(shared_ptr<Ship> target_ptr_)
{
	...
	// The raw pointer is temporarily used here to compare to "this" object.
	// There is no possible modification done with this raw pointer, and the
	// value is thrown away after the line executes, so there is no danger
	// of manipulating the smart pointer through the raw pointer.
...
// Describe the current state of the Warship
...
{
	double distance = cartesian_distance(get_location(), get_target()->get_location());
