...
void Ship::refuel()
{
	...
	// To accomidate for double precision, if the amount of fuel needed is small
	// simply consider the tank full
...
// may throw Error("Must be dead in the water!");
double Ship::refuel(double amount)
{
    if (ship_state != DEAD_IN_THE_WATER)
    {
        throw Error("Ship must be dead in the water!");
    }
    ...
    // To accomidate for double precision, if the amount of fuel needed is small
    // simply consider the tank full
    if (fuel_needed < 0.0005)
    {
        fuel = fuel_capacity;
        return 0;
    } 

	// Accept the minimum of the fuel required to fill the tank, and the
	// amount of fuel being provided
    double fuel_given = (fuel_needed < amount) ? fuel_needed : amount;
...
// will always throw Error("Cannot fuel another ship!");
void Ship::fuel_ship(shared_ptr<Ship>)
{
    throw Error("Cannot fuel another ship!");
...
// Return the maximum speed for the ship
